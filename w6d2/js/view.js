...
View.prototype.exercise4 = function () {
  ...
  for (let i = 0; i < $listItems.length; i++) {
    if (i%2 === 0) $listItems[i].append("rm");
  }
};
...
View.prototype.exercise5 = function () {
  ...
  $('.square').on("click", event => {
    ...
    alert($currSquare.attr("data-pos")); // $("li").on("click", annoy);
  });
...
View.prototype.exercise6 = function () {
  ...
  $listItems.eq(i).css("background-color", window._randomColorString());
  }
};
...
View.prototype.exercise7 = function(){
  ...
  $('.square').hover( event => {
    const $currSquare = $(event.currentTarget);
    console.log($currSquare.attr("background-color")); // $("li").on("click", annoy);
  });
};
